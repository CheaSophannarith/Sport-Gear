<script setup lang="ts">
    ...
    const imagePreview = ref<string | null>(props.carousel.image_url);
    ...
    const submit = () => {
        ...
        })).post(CarouselController.update.url(props.carousel.id), {
            forceFormData: true,
        });
    };
</script>
            ...
            <Card>
                <CardHeader>
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <CardTitle class="text-xl sm:text-2xl">Edit Carousel</CardTitle>
                            <p class="text-sm text-muted-foreground mt-1">
                                Update carousel details and settings
                            </p>
                        </div>
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <div class="space-y-4 order-1 lg:order-2">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div v-else
                                            ...
                                            <div class="text-center p-4">
                                                ...
                                                <p class="mt-2 text-sm text-muted-foreground">No image</p>
                                                <p class="text-xs text-muted-foreground mt-1">Upload a new image</p>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <Card class="bg-muted/50">
                                    <CardContent class="p-4 space-y-2">
                                        <h4 class="text-sm font-semibold">Carousel Information</h4>
                                        <div class="grid grid-cols-2 gap-2 text-xs">
                                            <div>
                                                <span class="text-muted-foreground">Created:</span>
                                                <p class="font-medium">{{ new Date(carousel.created_at).toLocaleDateString() }}</p>
                                            </div>
                                            <div>
                                                <span class="text-muted-foreground">Updated:</span>
                                                <p class="font-medium">{{ new Date(carousel.updated_at).toLocaleDateString() }}</p>
                                            </div>
                                        </div>
                                    </CardContent>
                                </Card>
                        ...
                        <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 border-t pt-6">
                            ...
                            <Button type="submit" :disabled="form.processing" class="w-full sm:w-auto">
                                {{ form.processing ? 'Updating...' : 'Update Carousel' }}
                            </Button>
                        </div>
                    </form>
                </CardContent>
